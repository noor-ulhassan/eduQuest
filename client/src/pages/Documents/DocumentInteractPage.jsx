...
import { 
  ArrowLeft, BrainCircuit, Loader2, CheckCircle, History, FileText, Target, Search, X
} from "lucide-react";
          ...
          <div className="lg:col-span-1 flex flex-col h-full">
            <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-lg flex items-center">
                    <FileText className="mr-2 w-5 h-5 text-blue-600" /> Source Document
                </h3>
                {activeQuote && (
          ...
          <div className="lg:col-span-1 flex flex-col h-full overflow-hidden">
            <div className="flex justify-between items-center bg-zinc-900 text-white p-4 rounded-xl mb-4 shadow-md">
               <div>
                   ...
                   <p className="text-zinc-400 text-sm">Score: {reviewingAttempt.percentage}%</p>
               </div>
            ...
            <div className="overflow-y-auto pr-2 space-y-4 pb-10">
              {reviewingAttempt.qaPairs.map((qa, idx) => (
                <Card key={idx} className={`border-l-4 shadow-sm ${qa.isCorrect ? "border-l-green-500" : "border-l-red-500"}`}>
                  <CardContent className="p-5">
                     <div className="flex justify-between items-start">
                         <p className="font-bold mb-2 text-zinc-800 w-[90%]">Q{idx + 1}. {qa.question}</p>
                         {qa.isCorrect ? <CheckCircle className="w-5 h-5 text-green-500" /> : <X className="w-5 h-5 text-red-500" />}
                     </div>
                     
                     <div className="mt-2 space-y-1 text-sm">
                         <p className="text-zinc-500">Your Answer: <span className={qa.isCorrect ? "text-green-600 font-bold" : "text-red-600 font-bold"}>{qa.userAnswer}</span></p>
                         {!qa.isCorrect && <p className="text-green-600 font-bold">Correct: {qa.correctAnswer}</p>}
                     </div>
