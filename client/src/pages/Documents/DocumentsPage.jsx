...
import {
  FileText, MessageSquare, HelpCircle, BookOpen, Plus,
...
function DocumentsPage() {
  ...
  useEffect(() => {
    if (!id) { setSelectedDoc(null); return; }
    setDocLoading(true);
  ...
  const goBack = () => {
    if (['chat', 'quiz', 'companion'].includes(view)) navigate(`/documents/${id}`);
  ...
  function Breadcrumb() {
    return (
      <div className="flex items-center gap-2 text-sm px-6 py-4 border-b border-white/10 flex-shrink-0">
        <button onClick={goBack} className="flex items-center gap-1.5 text-zinc-400 hover:text-white transition-colors">
          <ArrowLeft size={14} /> Back
        </button>
        <ChevronRight size={12} className="text-zinc-700" />
        <span className="text-zinc-400 hover:text-white cursor-pointer transition-colors" onClick={() => navigate('/documents')}>
          Documents
        </span>
        {selectedDoc && (
          <>
            <ChevronRight size={12} className="text-zinc-700" />
            <span className="text-zinc-300 font-medium truncate max-w-xs cursor-pointer hover:text-white transition-colors" onClick={() => navigate(`/documents/${id}`)}>
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-[#0a0a0a]">
        ...
        {showUpload && (
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="flex items-center gap-2 text-sm px-6 py-4 border-b border-white/10 flex-shrink-0">
              <button onClick={() => setShowUpload(false)} className="flex items-center gap-1.5 text-zinc-400 hover:text-white transition-colors">
                <ArrowLeft size={14} /> Back
              </button>
              <ChevronRight size={12} className="text-zinc-700" />
          ...
          <div className="flex-1 overflow-y-auto p-8">
            <div className="flex items-start justify-between mb-8">
              <div>
                ...
                <p className="text-sm text-zinc-400 mt-1">Upload PDFs to chat, quiz, and explore with AI</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {loading && [0, 1, 2].map((i) => (
                <div key={i} className="h-52 rounded-2xl bg-[#111111] border border-white/10 animate-pulse" />
              ))}

              {!loading && allDocs.map((doc) => {
            ...
            {docLoading ? (
              ...
              <div className="flex-1 overflow-y-auto flex flex-col items-center justify-center p-8">
                <div className="text-center mb-10">
                  ...
                  <p className="text-sm text-zinc-400 mt-1">Choose a mode to start working with this document</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 w-full max-w-3xl">
                  {[
                    { fid: 'chat',      label: 'Chat',      description: 'Ask questions and get instant answers from your document', icon: MessageSquare, color: 'text-indigo-400',  bg: 'bg-indigo-600/20',  hover: 'hover:border-indigo-500/40'  },
                    { fid: 'quiz',      label: 'Quiz',      description: 'Test your knowledge with AI-generated questions',          icon: HelpCircle,    color: 'text-emerald-400', bg: 'bg-emerald-600/20', hover: 'hover:border-emerald-500/40' },
                    { fid: 'companion', label: 'Companion', description: 'Read the PDF with live AI explanations alongside',         icon: BookOpen,      color: 'text-violet-400',  bg: 'bg-violet-600/20',  hover: 'hover:border-violet-500/40'  },
                  ].map(({ fid, label, description, icon: Icon, color, bg, hover }) => (
                    <button
                      key={fid}
