    ...
    doc = await UserQuestProgress.findOneAndUpdate(
      { userId, type },
      {
        $setOnInsert: {
          ...
          type,
          periodStart,
          quests: buildFreshQuests(questIds),
        },
      },
 ...
 */
export const onPlaygroundSolve = async (
  userId,
  { language, difficulty, usedHints },
) => {
  ...
  for (const q of daily.quests) {
    ...
    if (
      (q.id === "daily_dose" || q.id === "double_down") &&
  ...
  const saves = [];
  if (dailyChanged) {
    daily.markModified("quests");
    saves.push(daily.save());
  }
  if (weeklyChanged) {
    weekly.markModified("quests");
    saves.push(weekly.save());
  }
